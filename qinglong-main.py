...
class MiMotion():
   ...
   #发送酷推
    ...
    # 推送telegram
    @staticmethod
    def push_telegram(tg_bot_token, tg_user_id, title, content):
        print("Telegram 推送开始")
        send_data = {
            "chat_id": tg_user_id,
            "text": title + '\n\n' + content,
        ...
        response = requests.post(
            url='https://api.telegram.org/bot{0}/{1}'.format(tg_bot_token, "sendMessage"),
            data=send_data
        )
    ...
    def main(self):
        ...
        try:
            ...
            min_step = 20000
        try:
            max_step = int(self.check_item.get("max_step", 30000))
        except Exception as e:
...
if __name__ == "__main__":
    try:
        # 从环境变量读取配置
        config_str = os.getenv('MIMOTION_CONFIG')
        if not config_str:
            print("❌ 环境变量 MIMOTION_CONFIG 未设置")
            exit(1)
            
        try:
            datas = json.loads(config_str)
        except json.JSONDecodeError:
            print("❌ 环境变量 MIMOTION_CONFIG 格式错误，必须是有效的 JSON")
            exit(1)
        
        # 开启根据地区天气情况降低步数（默认关闭）
        open_get_weather = datas.get("OPEN_GET_WEATHER", "False")
        
        # 设置获取天气的地区（上面开启后必填）如：area = "宁波"
        area = datas.get("AREA", "NO")
        
        # 和风天气 Private KEY
        qweather = datas.get("QWEATHER", "")  # 修正了变量名
        ...
        if tg_bot_token and tg_user_id:
            MiMotion.push_telegram(tg_bot_token, tg_user_id, '【小米运动步数修改】', msg)
        else:
        ...
        if datas.get("POSITION"):
            ...
            # 这里需要调用企业微信推送方法
            # 注意：原代码中调用的是 run(msg)，但根据上下文应该是企业微信推送
            # 假设我们有一个企业微信推送方法 push_wecom(msg)
            # MiMotion(check_item=_check_item).push_wecom(msg)
            print("⚠️ 企业微信推送功能需要实现具体方法")
